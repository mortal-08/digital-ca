...
const sourceConfig: Record<string, { label: string; icon: any }> = {
  tax: { label: 'Income Tax', icon: Landmark },
  rbi: { label: 'RBI', icon: Scale },
  sebi: { label: 'SEBI', icon: Shield },
  gst: { label: 'GST', icon: Receipt },
  mca: { label: 'MCA', icon: TrendingUp },
};
...
export default function NewsUpdates() {
  ...
  const fetchNews = async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`${API_BASE}/api/news?t=${Date.now()}`);
      if (res.ok) {
